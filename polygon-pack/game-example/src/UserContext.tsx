...
export const UserProvider = ({ children }:any) => {
  ...
  const [accountType, setAccountType] = useState("basic") // Type of account connected, basic with login/password or wallet conneciton ("wallet")
  const [userLogin, setUserLogin] = useState(null); // Login of user (username, nickname)
  const [userPublicKey, setUserPublicKey] = useState(null); // Polygon wallet public address
  const [userSecretKey, setUserSecretKey] = useState(null); // Polygon wallet secret address (in case of custodial wallet)

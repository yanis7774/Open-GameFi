...
const paidGeneratorsObj: Generator[] = [
  { id: 'coffee', index: 0, name: 'Coffee Shop', baseCost: 0.1, baseProduction: 1, icon: <Coffee className="w-4 h-4 sm:w-6 sm:h-6" /> },
  { id: 'iceCream', index: 1, name: 'Ice Cream Parlor', baseCost: 0.5, baseProduction: 5, icon: <IceCream className="w-4 h-4 sm:w-6 sm:h-6" /> },
  { id: 'cake', index: 2, name: 'Cake Factory', baseCost: 1, baseProduction: 10, icon: <Cake className="w-4 h-4 sm:w-6 sm:h-6" /> },
]
...
export default function Game({}: { currency?: number, setCurrency: (value: number | ((prevValue: number) => number)) => void }) {
  ...
  const buyPaidGenerator = async (generator: Generator) => {
    if (accountType === "wallet") {
      ...
      try {
        ...
        sendMessage("updatePaidGenerator",{index: generator.index});
      } catch (error) {
